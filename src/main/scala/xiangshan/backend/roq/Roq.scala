...
class Roq(numWbPorts: Int) extends XSModule with HasCircularQueuePtrHelper {
  ...
  for (i <- 0 until numWbPorts) {
    when (io.exeWbResults(i).valid) {
      ...
      debug_microOp(wbIdx).predData := io.exeWbResults(i).bits.uop.predData
  ...
  if(!env.FPGAPlatform) {
    ...
    for (i <- 0 until CommitWidth) {
      ...
      val isMMIO = debug_exuDebug(idx).isMMIO && io.commits.valid(i)
      ...
      instType(i) := Mux(io.commits.valid(i), Cat(instTypeList), 0.U)
